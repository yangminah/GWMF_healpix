...
class Lindzen1981:
    """
    Implementation of a single instance of Lindzen 1981.
    The various _predict methods allow for computing L81 on a single profile
    or for multiple profiles, and in two different ways: looped or by iterating
    until there are no more breaking.
    """
    def __init__(
        self,
        direction: str = "NE",
    ...
    def _predict_multiple(self, data: n3, split: bool = False) -> dict | torch.Tensor:
        ...
        c_hat0 = torch.gather(
            c_hat, 1, data.source_levs.unsqueeze(1).repeat(1, 1, 1, self.nps)
        )
        ...
        levs = (
            torch.arange(data.nlev, dtype=torch.uint8)
            .unsqueeze(0)
            .unsqueeze(2)
            .unsqueeze(3)
        )
        ...
        levshape = list(sign_change.shape)
        levshape.remove(data.nlev)
        ...
        while torch.sum(breaking).item() > 0:
            ...
            F_c = torch.where(
                levs <= breaking_levels,
                breaking_cond.gather(
                    1, torch.where(breaking_levels == -1, 0, breaking_levels)
                ),
                F_c,
            )
            breaking = (F_c > breaking_cond) * above_source
    ...
    def _predict(self, datum: dict, split: bool = False) -> dict | torch.Tensor:
        ...
        breaking_cond = torch.where(
            levs <= find_lowest_true(sign_change), 0, breaking_cond
        )
        ...
        while torch.sum(breaking).item() > 0:
            ...
            F_c = torch.where(
                levs <= breaking_levels,
                breaking_cond.gather(
                    0, torch.where(breaking_levels == -1, 0, breaking_levels)
                ),
                F_c,
            )
            breaking = (F_c > breaking_cond) * above_source
    ...
    def _loop_predict(self, datum: dict, split: bool = False) -> dict | torch.Tensor:
        ...
        for lev in range(datum["source_levs"], -1, -1):
            ...
            MF[lev] = torch.sum(
                self.dc * c_hat0 * F_c * above_source[lev : lev + 1], dim=1
            ).squeeze()
    ...
    def _loop_predict_multiple(
        self, data: n3, split: bool = False
    ) -> dict | torch.Tensor:
        """
        ...
        for lev in range(start_loop, -1, -1):
            if lev < start_loop:
                ...
                sign_change = (
                    sign_change
                    | (c_hat[:, lev : lev + 1, :, :] != c_hat0)
                    * above_source[:, lev : lev + 1, :, :]
                )
                breaking_cond[:, lev : lev + 1, :, :] = torch.where(
                    sign_change, 0, breaking_cond[:, lev : lev + 1, :, :]  # ,
                )
            ...
            # Assign appropriate signs and sum MFs across phase speeds.
            MF[:, lev, :] = torch.sum(
                self.dc * c_hat0 * F_c * above_source[:, lev : lev + 1, :], dim=3
            ).squeeze()
