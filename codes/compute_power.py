"""
Compute powers, iterating for each month.
"""
import os
import argparse
import numpy as np
...
def wavenumber2flat(m, l, lmax):
    """
    Convert wavenumber (m,l) to its flattened index, lmax.
    """
    if np.isscalar(m):
        n = 0
        for i in range(m):
...
def main():
    """
    Compute powers from precomputed spherical harmonics coefficients.
    """
    ...
    for month_ind in range(task_id, task_id + 5):  # (5x12)
        ...
        for level_ind in range(n_height):
            ...
            # Get the remaining modes powers (for each l, m = 1, .., l;
            # and double the magnitudes for m=-l,...,-1)
